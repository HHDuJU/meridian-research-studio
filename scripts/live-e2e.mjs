...
// grounding.ts on trees that have it (S1 re-check plus the title check); support.ts claimSupport on older ones.
const { checkClaim } = await lib("src/lib/evidence/grounding.ts");
const { claimSupport: legacyClaimSupport } = await lib("src/lib/evidence/support.ts");
const claimSupport = checkClaim ?? legacyClaimSupport;
