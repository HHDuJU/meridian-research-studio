#!/usr/bin/env node
// validate-scenarios.mjs: checks Meridian usage-scenario files against SCENARIO_FORMAT.md (format version 1).
//
// Usage: node validate-scenarios.mjs <dir> [--forbidden <terms.txt>] [--index <INDEX.json>]
//   <dir>         folder whose *.json files are scenarios (a file named INDEX.json is skipped)
...
//   --index       write the bank index there ([{ id, title, field, level, family, requires, sha256 }],
//                 sha256 of the file bytes) when every file passes; nothing is written on a failure
//
...
//
// Schema key lists are copied from src/lib/ai.ts schemaFor() and DECISION_SCHEMA at 875fe59; the families,
// stages and issue codes from src/lib/types.ts and src/lib/contracts.ts at the same revision.
...
const STAGE_KEYS = {
  ...
  scan: ["query", "sourcesConsulted", "gradeOverall", "gradeRationale", "synthesis", "items", "summary"],
...
const STEP_SPECS = {
  ...
  illuminate: { required: ["stage"], optional: ["response", "responseText", "call", "omitKeys"] },
...
const EXPECT_KEYS = ["store", "screen", "issues", "export", "stage", "error"];
...
const TOP_OPTIONAL = ["earlyStop", "retrieval"];
...
// Calls fn(string, path) for every string value and fk(key, path) for every object key, depth first.
...
function validateFile(file, forbidden) {
  const r = { file: path.basename(file), id: "", level: "", field: "", family: "", steps: 0, stages: 0, calls: 0, fails: [], warns: [], sc: null, sha256: "" };
  ...
  if (sc.version !== FORMAT_VERSION) fail("version", `must be ${FORMAT_VERSION}`);
  ...
  const checkExpect = (ex, step, where) => {
    ...
    if (has(ex, "store")) {
      ...
      else for (const [p, v] of Object.entries(ex.store)) {
        ...
        if (has(ex.issues, "includes")) {
          if (!Array.isArray(ex.issues.includes) || ex.issues.includes.length === 0) fail(`${where}.issues.includes`, "must be a non-empty array of { path, code }");
            ...
            if (!isObj(it) || typeof it.path !== "string" || typeof it.code !== "string") fail(w, "needs string path and string code");
              ...
              // The scan mapper drops the model's sourcesConsulted text in every scan; that alone does not state a refusal.
  ...
  steps.forEach((step, i) => {
    ...
    switch (step.do) {
      ...
      case "illuminate": {
        ...
        const keys = STAGE_KEYS[step.stage];
        let omit = [];
        if (has(step, "omitKeys")) {
          if (!Array.isArray(step.omitKeys) || step.omitKeys.some((k) => typeof k !== "string")) fail(`${where}.omitKeys`, "must be an array of strings");
            ...
            omit = step.omitKeys;
            for (const k of omit) {
              if (!keys.includes(k)) fail(`${where}.omitKeys`, `"${k}" is not a top-level key of the ${step.stage} schema`);
        ...
        if (missing.length) fail(`${where}.response`, `missing top-level key(s) of the ${step.stage} schema: ${missing.join(", ")} (list deliberate omissions in omitKeys)`);
        const extra = Object.keys(resp).filter((k) => !keys.includes(k));
        if (extra.length) warn(`${where}.response`, `key(s) outside the ${step.stage} schema: ${extra.join(", ")}`);
        ...
        if (step.stage === "scan") {
          ...
          if (Array.isArray(resp.items)) {
            const nItems = resp.items.length;
            if (nItems >= LIMITS.scanItems[0] && nItems <= LIMITS.scanItems[1]) realisticScan = true;
            else warn(`${where}.response.items`, `${nItems} items; a realistic scan lists ${LIMITS.scanItems[0]} to ${LIMITS.scanItems[1]}`);
            resp.items.forEach((it, k) => {
              ...
              for (const f of ["title", "authors", "source", "keyFindings"]) {
                if (typeof it[f] !== "string" || !it[f].trim()) warn(`${where}.response.items[${k}].${f}`, "empty or not a string; realistic items name title, authors, journal and findings");
              }
            });
          }
        }
      ...
      case "change-source":
        ...
        else if (!scanTitles.has(step.record)) fail(`${where}.record`, "matches no item title in an earlier scan response");
  ...
  if (!realisticScan && !(earlyStop && jsonScans === 0)) fail("steps", `no scan response lists ${LIMITS.scanItems[0]} to ${LIMITS.scanItems[1]} items`);
...
function table(rows) {
  const head = ["FILE", "ID", "LVL", "FIELD", "FAMILY", "STEPS", "STAGES", "CALLS", "RESULT", "FAIL", "WARN"];
  const data = rows.map((r) => [r.file, r.id, String(r.level), r.field, r.family, String(r.steps), String(r.stages), String(r.calls), r.fails.length ? "FAIL" : "PASS", String(r.fails.length), String(r.warns.length)]);
...
function main() {
  ...
  if (args.index) {
    if (failed) console.log(`INDEX not written (${args.index}): ${failed} file(s) failed`);
      ...
      const index = rows
        .map((r) => ({ id: r.sc.id, title: r.sc.title, field: r.sc.field, level: r.sc.level, family: r.sc.family, requires: r.sc.requires, sha256: r.sha256 }))
      ...
      console.log(`INDEX written: ${args.index} (${index.length} entries)`);
    }
  }
