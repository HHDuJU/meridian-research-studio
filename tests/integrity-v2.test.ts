...
test("a manual identity check cannot verify; its result and note are recorded (a45 D37)", () => {
  const s = S().create({ family: null, setting: "s", rawNeed: "n" });
  ...
  const id = lead[0].id;
  const status = () => S().studies.find((x) => x.id === s.id)!.scan.items[0].provenance.status;
  assert.equal(S().changeSource(s.id, { id }, "status", "verified", "looked it up").ok, false);
  assert.equal(S().changeSource(s.id, { id }, "status", "retrieved", "looked it up").ok, false);
  assert.equal(status(), "unverified");
  S().changeSource(s.id, { id }, "status", "match", "title matches the publisher page");
  assert.equal(status(), "unverified");
  assert.equal(S().changeSource(s.id, { id }, "status", "mismatch", "").ok, false);
  assert.equal(S().changeSource(s.id, { id }, "status", "mismatch", "DOI resolves to another paper").ok, true);
...
test("ledger check re-runs S1 on the stored text: quotation and every number must resolve", () => {
  ...
  assert.equal(checkClaim(claim({}), study).status, "supported");
  assert.equal(checkClaim(claim({ text: "38% reached a 30% reduction.", passage: "thirty-eight percent reached a 30% reduction" }), study).status, "supported");
  const fabricatedNumber = checkClaim(claim({ text: "Pain fell by 4.5 points in 120 participants." }), study);
  assert.equal(fabricatedNumber.status, "unsupported");
  assert.equal(fabricatedNumber.blocking, true);
  assert.match(fabricatedNumber.message, /4\.5/);
  const invented = checkClaim(claim({ passage: "ketamine abolished pain in every patient" }), study);
  assert.equal(invented.status, "unsupported");
  assert.match(invented.message, /not an exact span/);
  assert.equal(checkClaim(claim({ kind: "inference", text: "Pooled 9.9" }), study).status, "not-source-derived");
  assert.equal(checkClaim(claim({ sourceIds: ["ev-missing1"] }), study).blocking, true);
});
...
test("a decision cannot rest on a claim its source text does not support", () => {
  ...
  assert.match(support.reason, /claim c-fab: .*4\.5/);
...
test("appraisal quarantines an unsupported claim with a visible issue and keeps the supported one", () => {
  const { study, id } = studyWithRecord(ABSTRACT);
  ...
  assert.deepEqual(claims.map((c) => c.id), ["c1"]);
  const q = out.stagePatch.quarantine as { claims: Claim[] };
  assert.ok(q.claims.some((c) => c.text === "Pain fell by 48 points."));
  assert.ok(out.issues.some((i) => i.code === "unsupported" && /48/.test(i.message)));
});
...
test("fractions in words are approximate figures, not numbers the source must contain", () => {
  ...
  const wrong = checkClaim({ ...base, text: "The database captured about three quarters (81 percent) of events." }, study);
  assert.equal(wrong.status, "unsupported");
  assert.match(wrong.message, /81/);
});
...
test("appraisal batches after the first apply; 'local-fact' claims keep their kind (live run)", () => {
  ...
  }, studyRevision(fresh()), { appraisedRecordIds: batches[0], appraisalBatch: { index: 1, of: batches.length } });
  ...
  }, expected, { appraisedRecordIds: batches[1], appraisalBatch: { index: 2, of: batches.length } });
...
test("a stored text that does not belong to its record blocks claims that cite it (live run)", () => {
  ...
  const r = checkClaim(claim, study);
  ...
  assert.equal(checkClaim({ ...claim, sourceIds: [ok.id], text: "Pain fell by 2.1 points.", passage: "Pain scores fell by 2.1 points" }, ok.study).status, "supported");
});
...
test("id marking works on a copy and never rewrites investigator-authored objects", () => {
  // The appraisal merge carries stored claim objects into the patch. Marking must not change the stored
  // study through a shared reference (the stale-batch defect of the 2026-09-22 live run), and an
  // investigator's own text is not rewritten even when it mentions an id-like token Meridian does not know.
  const s = S().create({ family: "qi-pdsa", setting: "s", rawNeed: "Handover PCA errors." });
  S().mergeStage(s.id, "scan", {
    claims: [
      { id: "inv-1", text: "Our 2025 audit (see ev-9zz999 in the ward folder) found 41 incidents.", kind: "local-fact", sourceIds: [], uncertainty: "moderate", origin: "investigator" },
      { id: "c-old", text: "An earlier model claim.", kind: "inference", sourceIds: [], uncertainty: "moderate", origin: "model" },
    ],
  });
  const fresh = () => S().studies.find((x) => x.id === s.id)!;
  ...
  const out = applyAiResult("scan", {
    annotations: [],
    claims: [{ id: "c-new", text: "A model claim that cites dec-7qq777.", kind: "inference", sourceIds: [], uncertainty: "moderate" }],
  }, "qi-pdsa", fresh(), { appraisedRecordIds: [] });
  assert.equal(out.ok, true);
  assert.equal(JSON.stringify(fresh()), before);
  const claims = out.stagePatch.claims as Claim[];
  assert.equal(claims.find((c) => c.id === "inv-1")?.text, "Our 2025 audit (see ev-9zz999 in the ward folder) found 41 incidents.");
  assert.match(claims.find((c) => c.id === "c-new")?.text ?? "", /⟦unresolved:dec-7qq777⟧/);
  assert.ok(out.issues.some((i) => i.code === "unresolved-reference" && i.value === "dec-7qq777"));
});

test("re-appraisal replaces the model's unsourced claims; later batches of a run keep them", () => {
  const inference = (id: string, text: string): Claim => ({ id, text, kind: "inference", sourceIds: [], uncertainty: "moderate", origin: "model" });
  const investigator: Claim = { id: "inv-1", text: "Our ward logged 41 incidents.", kind: "local-fact", sourceIds: [], uncertainty: "moderate", origin: "investigator" };
  const existing = [inference("c1", "An earlier inference."), investigator];
  const rerun = mergeAppraisedClaims(existing, [inference("c1", "A new inference.")], new Set(["ev-a"]), { startsRun: true });
  assert.deepEqual(rerun.claims.map((c) => [c.id, c.text]), [["inv-1", "Our ward logged 41 incidents."], ["c1", "A new inference."]]);
  assert.deepEqual(rerun.superseded.map((c) => c.text), ["An earlier inference."]);
