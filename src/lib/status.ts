...
export function studyStatus(study: Study): StudyStatus {
  ...
  const checks = (study.scan.claims ?? []).map((c) => claimSupport(c, study));
  const claims = {
    total: checks.length,
    supported: checks.filter((c) => c.status === "supported" || c.status === "close-paraphrase" || c.status === "no-passage").length,
