...
interface StudioState {
  ...
  illuminateApply: (
    ...
    options?: { appraisedRecordIds?: string[] },
...
/** Statuses an investigator may set by hand. "verified" and "retrieved" need a recorded check or search. */
export const MANUAL_SOURCE_STATUSES = ["unverified", "mismatch", "check-failed", "access-blocked"] as const;
  ...
  persist(
    (set, get) => ({
      ...
      changeSource: (id, record, field, value) => {
        ...
        if (field === "status" && !(MANUAL_SOURCE_STATUSES as readonly string[]).includes(String(value))) {
          const reason = `status "${String(value)}" cannot be set by hand; it needs a recorded registry check or a real search`;
          get().log(id, { id: uid("audit"), at: nowIso(), kind: "note", stage: "scan", actor: "system", summary: `Source ${item.id} status change refused: ${reason}.` });
        ...
        const items = s.scan.items.map((it) => {
          ...
          if (field === "status") {
            const status = String(value) as EvidenceItem["provenance"]["status"];
            // A hand-set status is recorded as a manual check so the trail shows who set it and when.
            const manual: SourceCheck = {
              id: uid("chk"),
              at: nowIso(),
              provider: "manual",
              identifier: it.doi ?? it.pmid ?? it.id,
              result: status === "mismatch" ? "mismatch" : status === "check-failed" ? "error" : status === "access-blocked" ? "blocked" : "unresolved",
              note: `set by the investigator: ${it.provenance.status} -> ${status}`,
            };
            return { ...it, provenance: { ...it.provenance, status, checks: [...(it.provenance.checks ?? []), manual] } };
          }
